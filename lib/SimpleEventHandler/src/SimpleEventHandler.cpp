...
void SimpleEventHandler::newMessage(char *msg, peer_info *p)
{
  ...
  // // parsing msg
  ulong msgId;
  // auto res = sscanf(msg, "%lu;%s", &msgId, &msgText);
  // printf("%d\n", res);
  // if (res != 2)
  //   return; // invalid message format
  // // printf("SimpleEventHandler::newMessage: parsed msgId=%lu, msgText='%s', res=%d\n"
  // //   , msgId, msgText, res);
  // printf("%lu\n", msgId);
  // printf("%s\n", msgText);
  ...
  msgId = strtoul(msg, NULL, 10); // parse msg from to msgId
  ...
  // printf("SimpleEventHandler::newMessage finished: msgText='%s'\n", msgText);
}
