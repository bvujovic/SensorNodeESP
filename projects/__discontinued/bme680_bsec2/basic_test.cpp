...
void setup(void)
{
    ...
    for (uint8_t i = 0; i < NUM_OF_SENS; i++)
    {
        ...
        if (!envSensor[i].begin(BME68X_I2C_INTF, comm_mux_read, comm_mux_write, comm_mux_delay, &communicationSetup[i]))
        {
            Serial.println("Could not initialize BME68X sensor " + String(i));
        ...
        {
            Serial.println("Could not update BSEC subscription for sensor " + String(i));
            checkBsecStatus(envSensor[i]);
        }
...
void loop(void)
{
    ...
    */
    for (sensor = 0; sensor < NUM_OF_SENS; sensor++)
    {
        ...
        {
            Serial.println("Could not run BSEC for sensor " + String(sensor));
